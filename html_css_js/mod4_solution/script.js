...
( () => {
  ...
  //2.b. for JHU Course
  console.log("**** 2B ****");
  function mapNamesFunc(name) {
    return name.charAt(0).toLowerCase() === 'j' ?
      byeSpeaker.speakSimple(name) : helloSpeaker.speakSimple(name);
  }
  mapArray = [];
  names.map(name => mapArray.push(mapNamesFunc(name)));
  mapArray.forEach(name => console.log(name));
  ...
  for(var i in intiialValue.bye){
    console.log(intiialValue.bye[i]);
  }
